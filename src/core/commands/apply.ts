...
import { aimRefusal } from "../rules/targeting.js";
...
function validateAim(
  ...
  const refusal = aimRefusal(state, unit, ability, target, unit.position);
  return refusal === null ? null : commandError(refusal.code, refusal.message);
}
